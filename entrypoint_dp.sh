...
# RoboTwin DP (Diffusion Policy) Model Evaluation Container Entrypoint
...
show_help() {
    cat << EOF
RoboTwin DP (Diffusion Policy) Model Evaluation Container

Usage:
    docker run [docker-options] robotwin-dp-eval [task_name] [task_config] [ckpt_setting] [expert_data_num] [seed] [gpu_id]
...
Notes:
    ...
    - Model should be placed in policy/DP/dp_ckpt/dp-{task_name}/{ckpt_setting}-{expert_data_num}/
...
echo "Starting RoboTwin DP evaluation..."
echo "Task: $task_name"
...
# 检查模型目录
model_path="/RoboTwin/policy/DP/dp_ckpt/dp-${task_name}/${ckpt_setting}-${expert_data_num}"
if [ ! -d "/models" ]; then
