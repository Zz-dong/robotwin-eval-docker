...
# RoboTwin DP3 (3D Diffusion Policy) Model Evaluation Container Entrypoint
...
show_help() {
    cat << EOF
RoboTwin DP3 (3D Diffusion Policy) Model Evaluation Container

Usage:
    docker run [docker-options] robotwin-dp3-eval [task_name] [task_config] [ckpt_setting] [expert_data_num] [seed] [gpu_id]
...
Required Docker volumes:
    ...
    - Results directory: -v /host/to/results:/results

Notes:
    - Assets must be downloaded first using the official download script
    - The container requires GPU access (--gpus all)
    - Model should be placed in policy/DP3/dp3_ckpt/dp3-{task_name}/{ckpt_setting}-{expert_data_num}/
    - Results will be saved to the mounted results directory
    - DP3 requires point cloud data (data_type/pointcloud should be true during data collection)
...
echo "Starting RoboTwin DP3 evaluation..."
echo "Task: $task_name"
...
# 检查模型目录
model_path="/RoboTwin/policy/DP3/dp3_ckpt/dp3-${task_name}/${ckpt_setting}-${expert_data_num}"
if [ ! -d "/models" ]; then
